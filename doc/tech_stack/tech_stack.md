...
### Styling — **SCSS Modules** (`sass`) + design tokens
**Why:** SCSS gives us nesting, `@use`, `@mixin`, partials — all useful as the styles folder grows past the current ~10 files. Vite supports SCSS natively when `sass` is installed; nothing else to configure. Files become `*.module.scss` (same scoping as `.module.css`, no behavioural change). Tokens still live in `src/index.css :root` as plain CSS custom properties — SCSS variables are for local computed values (e.g. `$step: 4px`) and shouldn't shadow the design tokens.
**Alternative considered:** plain CSS Modules (works, but we'd hand-roll the patterns SCSS gives for free); PostCSS with `postcss-nesting` (slimmer than SCSS but less ergonomic and we lose mixins).
**Migration:** rename `*.module.css` → `*.module.scss` as you touch each component. Both extensions can coexist during the migration. New files use `.module.scss`.
...
| **Tailwind** *(may revisit — see note below)* | We picked SCSS Modules + tokens as the styling primitive. Tailwind would either duplicate the token system (`bg-blue-500` competing with our warm palette) or get reduced to `@apply` shorthand inside SCSS (which loses Tailwind's main JSX-utility benefit). If the team later wants JSX-utility ergonomics, the path is **Tailwind v4 with a `@theme` block exposing *only* our existing tokens** (`bg-primary`, `text-ink`, `border-line`, etc., and nothing else) — so Tailwind becomes a typing convenience over our tokens, not a parallel design system. Decide explicitly before adding. |
...
- **SCSS migration.** Existing `.module.css` files stay until touched, then rename to `.module.scss` in the same change. New files use `.module.scss` from the start. `sass` is the only new devDep.
...
- **Add for V1 (🟡):** **react-hot-toast**, **sass** (SCSS Modules), React Router, RTK Query (already in toolkit, just enable), react-hook-form + zod, date-fns + date-fns-tz, recharts, @tabler/icons-react, Sentry, PostHog, Vitest + Playwright, Prettier, husky + lint-staged.
- **Skip for V1 (⏸️):** SSR, GraphQL, CSS-in-JS, **Tailwind** *(unless we go hybrid — see §3)*, UI kits, WebSockets, i18n, 2FA, Storybook.
